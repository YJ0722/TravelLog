...
public class MemoDAO {
    ...
    public Cursor findByLogNo(int log_no) {
        Log.d("findByLogNo dao","&&&&&&&&&&&&");
        SQLiteDatabase db = mHelper.getReadableDatabase();

        Cursor c = db.rawQuery("select * from " + Memo.class.getSimpleName() + " where log_no = " + String.valueOf(log_no), null);
        Log.d("&&&&&&&&&&&&&","&&&&&&&&&&&&");
        return c;
    }
    public Cursor findByMemoNo(int memoNo) {
        SQLiteDatabase db = mHelper.getReadableDatabase();

        Cursor c = db.rawQuery("select * from " + Memo.class.getSimpleName() + " where _id = " + String.valueOf(memoNo), null);
    ...
    public boolean delete(int id) {
         ...
         */
        int affectedRowsCount = db.delete(Memo.class.getSimpleName(), BaseColumns._ID + " = ? ", new String[]{String.valueOf(id)});
