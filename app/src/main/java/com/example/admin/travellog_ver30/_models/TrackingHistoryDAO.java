...
public class TrackingHistoryDAO {
    ...
    public Cursor findAll() {
         ...
         */
        return db.query(TrackingHistory.class.getSimpleName(), null, null, null, null, null, "_id" + " ASC");
    }
    ...
    public Cursor findByTravelNo(int travel_no) {
        ...
        Cursor c = db.rawQuery("select * from " + TrackingHistory.class.getSimpleName() + " where travel_no = " + String.valueOf(travel_no) , null);

        return c;
    }

    public Cursor lastLogNo(int travel_no) {
        SQLiteDatabase db = mHelper.getReadableDatabase();

        Cursor c = db.rawQuery("select _id from " + TrackingHistory.class.getSimpleName()
                + " order by _id ", null);
    ...
    public Cursor findByLogNo(int travel_no, int log_no) {
        Log.d("findByLogNo dao","&&&&&&&&&&&&");
        SQLiteDatabase db = mHelper.getReadableDatabase();

        Cursor c = db.rawQuery("select * from " + TrackingHistory.class.getSimpleName() + " where travel_no = " + String.valueOf(travel_no) + " and _id = " + String.valueOf(log_no), null);
        Log.d("&&&&&&&&&&&&&","&&&&&&&&&&&&");
        Log.d("cursor 내용 : " , String.valueOf(c.getCount()));
        Log.d("&&&&&&&&&&&&&","&&&&&&&&&&&&");
        return c;
    }
    public Cursor findByOnlyLogNo(int log_no) {
        Log.d("findByOnlyLogNo dao","logNo 확인 : " + String.valueOf(log_no));
        SQLiteDatabase db = mHelper.getReadableDatabase();

        Cursor c = db.rawQuery("select * from " + TrackingHistory.class.getSimpleName() + " where _id = " + String.valueOf(log_no), null);

        Log.d("c커서 크기 확인", String.valueOf(c.getCount()));
        return c;
    }
